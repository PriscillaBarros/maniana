...
public class DragSortListView extends ListView {
    ...
    private void measureItemAndGetHeights(int position, View item, ItemHeights heights) {
        ...
        boolean isHeadFoot = position < getHeaderViewsCount()
        ...
        if (height > 0) {
            ...
            int wspec = ViewGroup.getChildMeasureSpec(mWidthMeasureSpec, getListPaddingLeft()
                    + getListPaddingRight(), lp == null ? ViewGroup.LayoutParams.FILL_PARENT
                    : lp.width);
    ...
    /**
     * This function works for arbitrary positions (could be off-screen). If requested position is
     * off-screen, this function calls <code>getView</code> to get height information.
     * 
    ...
    /**
     * Get the shuffle edge for item at position when top of item is at y-coord top
     * 
     * @param position
     * @param top
     * @param height Height of item at position. If -1, this function calculates this height.
     * 
     * @return Shuffle line between position-1 and position (for the given view of the list; that
     *         is, for when top of item at position has y-coord of given `top`). If floating View
     *         (treated as horizontal line) is dropped immediately above this line, it lands in
     ...
     * from the list.
     * 
     * @param remove Remove the dragged item from the list. Calls a registered DropListener, if one
     *        exists.
     * 
     ...
     * through to {@link #startDrag(int,View,int,int,int)} after obtaining the floating View from
     * the FloatViewManager.
     * 
     * @param position Item to drag.
     * @param dragFlags Flags that restrict some movements of the floating View. For example, set
     *        <code>dragFlags |= 
     * ~{@link #DRAG_NEG_X}</code> to allow dragging the floating View in all directions except off
     ...
     *         if we are not currently in a touch event, there is no registered FloatViewManager, or
     *         the FloatViewManager returns a null View.
     */
     ...
     * 
     * @return True if the drag was started, false otherwise. This <code>startDrag</code> will fail
     *         if we are not currently in a touch event, <code>floatView</code> is null, or there is
     *         a drag in progress.
     */
    ...
    public interface FloatViewManager {
         ...
         *        Views; thus, if you want to call {@link ListView#getChildAt(int)}, you will need
         ...
         *        View would like to appear, given the current touch location and the offset
         *        provided in {@link DragSortListView#startDrag}.
     ...
     * items are draggable, where the drag init mode determines how items are lifted (see {@link
     * setDragInitMode(int)}). If disabled, items cannot be dragged.
     * 
     ...
     * {@link BaseAdapter#notifyDataSetChanged()} or something like it. When an item removal occurs,
     ...
     * {@link BaseAdapter#notifyDataSetChanged()} or something like it in your implementation.
     * 
     * @author heycosmo
     * 
     */
